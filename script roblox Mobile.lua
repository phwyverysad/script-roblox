...
local initialConfig = {
    ...
    WalkSpeed = 1, WSToggle = false, JumpPower = 1, JPToggle = false, InfJump = false, FlyToggle = false, FlyNoclip = true, FlySpeed = 100, Noclip = false, InfZoom = true, ZoomNoclip = true, InvisToggle = false, FOVToggle = false, FOVView = 70, FOVColor_C3 = Color3.fromRGB(30,161,255),
...
-- [ RAYFIELD WRAPPER FOR MACLIB ]
local Rayfield = loadstring(game:HttpGet('https://sirius.menu/rayfield'))()
local MacLib = {
    Window = function(self, cfg)
        local win = Rayfield:CreateWindow({
            Name = cfg.Title or "Phwyverysad",
            LoadingTitle = "Loading...",
            LoadingSubtitle = cfg.Subtitle or "Mobile Version",
            Theme = "Default",
            ConfigurationSaving = { Enabled = false }
        })
        return {
            SetState = function(s, v) Rayfield:SetVisibility(v) end,
            Notify = function(s, n) Rayfield:Notify({Title=n.Title or "Notification", Content=n.Description or "", Duration=n.Lifetime or 5}) end,
            Dialog = function(s, d) 
                if d.Buttons and d.Buttons[1] and d.Buttons[1].Callback then
                    task.delay(0.1, d.Buttons[1].Callback)
                end
            end,
            Unload = function(s) Rayfield:Destroy() end,
            GlobalSetting = function(s, gCfg)
                if not s._globalTab then
                    s._globalTab = win:CreateTab("Global Settings", 4483362458)
                end
                local tgl = s._globalTab:CreateToggle({
                    Name = gCfg.Name or "Setting",
                    CurrentValue = gCfg.Default,
                    Flag = (gCfg.Name or "Setting") .. tostring(math.random(1,100000)),
                    Callback = gCfg.Callback or function() end
                })
                return { SetVisibility = function() end, Set = function(ss, v) tgl:Set(v) end }
            end,
            TabGroup = function(s)
                return {
                    Tab = function(s2, t)
                        local rTab = win:CreateTab(t.Name or "Tab", 4483362458)
                        local function createSectionObj(gNameOrCfg)
                            local sectionName = ""
                            if type(gNameOrCfg) == "string" then sectionName = gNameOrCfg end
                            if sectionName ~= "" then rTab:CreateSection(sectionName) end
                            return {
                                Toggle = function(s4, c)
                                    local tgl = rTab:CreateToggle({
                                        Name = c.Title or c.Name or "",
                                        CurrentValue = c.Default,
                                        Flag = (c.Title or c.Name or "") .. tostring(math.random(1,100000)),
                                        Callback = c.Callback or function() end
                                    })
                                    return { SetVisibility = function() end, Set = function(ss, v) tgl:Set(v) end }
                                end,
                                Button = function(s4, c)
                                    local btn = rTab:CreateButton({
                                        Name = c.Name or c.Title or "",
                                        Callback = c.Callback or function() end
                                    })
                                    return { SetVisibility = function() end }
                                end,
                                Dropdown = function(s4, c)
                                    local curOpt = nil
                                    if type(c.Default) == "table" then curOpt = c.Default elseif c.Default then curOpt = {c.Default} else curOpt = {""} end
                                    local dd = rTab:CreateDropdown({
                                        Name = c.Title or c.Name or "",
                                        Options = c.Options or {""},
                                        CurrentOption = curOpt,
                                        Flag = (c.Title or c.Name or "") .. tostring(math.random(1,100000)),
                                        Callback = function(opts) if c.Callback and type(opts)=="table" then c.Callback(opts[1]) end end
                                    })
                                    return { 
                                        SetVisibility = function() end, 
                                        SetOptions = function(ss, opts) dd:Refresh(opts, true) end, 
                                        Set = function(ss, opt) dd:Set({opt}) end,
                                        ClearOptions = function() dd:Refresh({""}, true) end,
                                        InsertOptions = function(ss, opts) dd:Refresh(opts, true) end,
                                        UpdateSelection = function(ss, opt) dd:Set({opt}) end
                                    }
                                end,
                                Slider = function(s4, c)
                                    local sld = rTab:CreateSlider({
                                        Name = c.Title or c.Name or "",
                                        Range = {c.Min or c.Minimum or 0, c.Max or c.Maximum or 100},
                                        Increment = c.Step or 1,
                                        CurrentValue = c.Default or c.Min or c.Minimum or 0,
                                        Flag = (c.Title or c.Name or "") .. tostring(math.random(1,100000)),
                                        Callback = c.Callback or function() end
                                    })
                                    local currentVal = sld.CurrentValue
                                    task.spawn(function()
                                        while task.wait(0.05) do
                                            if sld.CurrentValue ~= nil and sld.CurrentValue ~= currentVal then
                                                currentVal = sld.CurrentValue
                                                if c.Callback then c.Callback(currentVal) end
                                            end
                                        end
                                    end)
                                    return { SetVisibility = function() end, Set = function(ss, v) sld:Set(v) end }
                                end,
                                Colorpicker = function(self4, c)
                                    local cp = rTab:CreateColorPicker({
                                        Name = c.Title or c.Name or "",
                                        Color = c.Default or Color3.new(1,1,1),
                                        Flag = (c.Title or c.Name or "") .. tostring(math.random(1,100000)),
                                        Callback = c.Callback or function() end
                                    })
                                    return { SetVisibility = function() end, Set = function(ss, v) cp:Set(v) end }
                                end,
                                Keybind = function(self4, c)
                                    local defaultKey = "RightControl"
                                    if c.Default then defaultKey = typeof(c.Default) == "EnumItem" and c.Default.Name or c.Default end
                                    local kb = rTab:CreateKeybind({
                                        Name = c.Title or c.Name or "",
                                    ...
                                    local currentBind = kb.CurrentKeybind
                                    task.spawn(function()
                                        while task.wait(0.1) do
                                            if kb.CurrentKeybind ~= nil and kb.CurrentKeybind ~= currentBind then
                                                currentBind = kb.CurrentKeybind
                                                if c.onBinded then c.onBinded(currentBind) end
                                            end
                                        end
                                    end)
                                    return { SetVisibility = function() end, Set = function(ss, v) kb:Set(typeof(v) == "EnumItem" and v.Name or v) end }
                                end,
                                Label = function(self4, c)
                                    local lb = rTab:CreateLabel(c.Title or c.Name or "")
                                    return { SetVisibility = function() end, Set = function(ss, v) lb:Set(v) end }
                                end,
                                Paragraph = function(self4, c)
                                    return { SetVisibility = function() end, Set = function(ss, t, d) end }
                                end,
                                Spacer = function(self4) return { SetVisibility = function() end } end,
                                Divider = function(self4) rTab:CreateDivider(); return { SetVisibility = function() end } end,
                                Input = function(self4, c)
                                    local ip = rTab:CreateInput({
                                        Name = c.Title or c.Name or "",
                                        CurrentValue = c.Default or "",
                                        PlaceholderText = "Input here...",
                                        RemoveTextAfterFocusLost = false,
                                        Flag = (c.Title or c.Name or "") .. tostring(math.random(1,100000)),
                                        Callback = c.Callback or function() end
                                    })
                                    return { SetVisibility = function() end, Set = function(ss, v) ip:Set(v) end }
                                end,
                                Header = function(self4, c)
                                    return { SetVisibility = function() end, Set = function(ss, v) end }
                                end,
                                SubLabel = function(self4, c)
                                    return { SetVisibility = function() end, Set = function(ss, v) end }
                                end
                            }
                        end
                        return {
                            Group = function(s3, g) return createSectionObj(g) end,
                            Section = function(s3, c) return createSectionObj(c) end
                        }
                    end
                }
            end
...
local RTX_System = { Cleanup = nil, OrigProps = {}, OrigChildren = {} }

function RTX_System.Enable()
    if RTX_System.Cleanup then return end
    ...
    do
        ...
        for _, v in pairs(Lighting:GetChildren()) do
            if not v:IsA("Sky") then
                table.insert(RTX_System.OrigChildren, v)
                v.Parent = nil
            end
        end
        ...
        if not RTX_System.OrigProps.BackedUp then
            RTX_System.OrigProps = {
                BackedUp = true,
                Technology = pcall(function() return Lighting.Technology end) and Lighting.Technology or Enum.LightingTechnology.ShadowMap,
        ...
        pcall(function() Lighting.Technology = Enum.LightingTechnology.Future end)
        ...
        local charConn = pl.CharacterAdded:Connect(CharacterSystem.onChar)
        ...
        RTX_System.Cleanup = function()
            if hbConn then hbConn:Disconnect() end
            if charConn then charConn:Disconnect() end
            if RainSystem and RainSystem.folder then RainSystem.folder:Destroy() end
            if CinematicGui then CinematicGui:Destroy() end
            if TimeDisplay then TimeDisplay:Destroy() end
            if WeatherDisplay then WeatherDisplay:Destroy() end
            
            for _, v in pairs(Lighting:GetChildren()) do
                if v:IsA('BloomEffect') or v:IsA('ColorCorrectionEffect') or v:IsA('SunRaysEffect') or v:IsA('BlurEffect') or v:IsA('DepthOfFieldEffect') or v:IsA('Sky') then
                    v:Destroy()
                end
            end
            
            for _, v in ipairs(RTX_System.OrigChildren) do
                if v then v.Parent = Lighting end
            end
            table.clear(RTX_System.OrigChildren)
            
            if RTX_System.OrigProps and RTX_System.OrigProps.BackedUp then
                for k, v in pairs(RTX_System.OrigProps) do
                    if k ~= "BackedUp" then
            ...
            if ResetSky then ResetSky() end
        end
    end
end

function RTX_System.Disable()
    if RTX_System.Cleanup then
        RTX_System.Cleanup()
        RTX_System.Cleanup = nil
    end
end
...
function SetRTX(on)
    if on then
        ...
        pcall(function() UpdateToggleUIFromKeybind("RTX_Enabled") end)
        ShowToast("⛔ ปิด Ray Tracing แล้ว", Colors.Red)
    end
end
    ...
    if on then
        Conns.WS_Loop = RunService.RenderStepped:Connect(function(dt)
            ...
            if h and hrp and h.MoveDirection.Magnitude > 0 then
                ...
                hrp.Velocity = Vector3.new(h.MoveDirection.X * (h.WalkSpeed * Config.WalkSpeed), hrp.Velocity.Y, h.MoveDirection.Z * (h.WalkSpeed * Config.WalkSpeed))
            end
        end)
    else
...
function SetJumpPower(on)
    ...
    if on then
        Conns.JP_Loop = RunService.Heartbeat:Connect(function()
            ...
            if h then
                h.UseJumpPower = true
                local baseJump = (_G._PwyvOrig and _G._PwyvOrig.JumpPower) or 50
                h.JumpPower = baseJump * Config.JumpPower
            end
        end)
    else
        local h = LocalPlayer.Character and LocalPlayer.Character:FindFirstChildOfClass("Humanoid")
...
function BuildAllTabs()
    ...
    Section_Movement:Slider({
        Name = TL("Speed Multiplier", "ตัวคูณความเร็ว"),
        Default = Config.WalkSpeed,
        Minimum = 1,
        Maximum = 20,
        Step = 1,
        DisplayMethod = "Value",
        Callback = function(v) Config.WalkSpeed = ApplySliderStep(v, 1, 20, false); if Config.WSToggle then SetWalkSpeed(true) end end
    })
    ...
    Section_Movement:Slider({
        Name = TL("Jump Multiplier", "ตัวคูณกระโดด"),
        Default = Config.JumpPower,
        Minimum = 1,
        Maximum = 20,
        Step = 1,
        DisplayMethod = "Value",
        Callback = function(v) Config.JumpPower = ApplySliderStep(v, 1, 20, false); if Config.JPToggle then SetJumpPower(true) end end
    })
    ...
    do
        ...
        Section_FlingMain:Header({ Name = TL("MAIN SETTINGS", "ตั้งค่าหลัก") })
        Section_FlingButtons:Header({ Name = TL("VISIBLE FLOATING BUTTONS", "ปุ่มลอยที่แสดง") })
        Section_FlingProtection:Header({ Name = TL("GODMODE & ANTI-FALL", "อมตะ & ป้องกันการตก") })

        Section_FlingMain:Toggle({
            Name = TL("Enable Fling", "เปิดฟลิง"),
        ...
        Section_FlingMain:Button({
            Name = TL("Execute / Stop Fling", "เริ่ม / หยุดฟลิง"),
            Callback = function() FlingFire() end
        })

        for _, name in ipairs(Fling.Names) do
            Section_FlingButtons:Toggle({
                Name = TL("Button: " .. name, "ปุ่ม: " .. name),
        ...
        Section_FlingProtection:Toggle({
            Name = TL("Anti-Fall Damage", "ป้องกันตกจากที่สูง (Anti-Fall)"),
            Default = Fling.State.antiFallEnabled,
            Callback = FlingSetAntiFall
        })
        Section_FlingProtection:Toggle({
            Name = TL("Godmode (Anti-Instakill)", "อมตะ (Godmode)"),
            Default = Fling.State.godEnabled,
            Callback = FlingSetGod
        })
        Section_FlingProtection:Toggle({
            Name = TL("Anti-Fling Protection", "ป้องกันการถูกฟลิง (Anti-Fling)"),
    ...
    Toggles["RTX_Enabled"] = Section_RayTracing:Toggle({
        Name = TL("Ray Tracing", "เรย์เทรซซิ่ง"),
        Default = Config.RTX_Enabled,
        Callback = function(v)
            if Config.RTX_Enabled ~= v then
                SetRTX(v)
            elseif v and not RTXLoaded then
                SetRTX(true)
            end
        end
    }, "RTX_Enabled")
    ...
    local function PerformServerHop(mode)
        local statusStr = "กำลังค้นหาเซิร์ฟเวอร์..."
        if mode == "HighestPlayer" then statusStr = "กำลังค้นหาเซิร์ฟเวอร์คนเยอะ..."
        elseif mode == "LowestPlayer" then statusStr = "กำลังค้นหาเซิร์ฟเวอร์คนน้อย..."
        elseif mode == "LowestPing" then statusStr = "กำลังค้นหาเซิร์ฟเวอร์ปิงน้อย..."
        elseif mode == "LowestPingAndPlayer" then statusStr = "กำลังค้นหาปิงน้อย + คนน้อย..."
        end
        ShowToast(TL("Searching for server...", statusStr), Colors.PrimaryBlue)
        
        task.spawn(function()
            pcall(function()
                local HttpService = game:GetService("HttpService")
                local sort = (mode == "HighestPlayer") and "Desc" or "Asc"
                local req = request or http_request or (syn and syn.request) or (fluxus and fluxus.request)
                
                local validServers = {}
                local cursor = ""
                local pagesToFetch = (mode == "HighestPlayer") and 1 or 5
                local pagesFetched = 0
                
                while pagesFetched < pagesToFetch do
                    local url = "https://games.roblox.com/v1/games/" .. tostring(game.PlaceId) .. "/servers/Public?sortOrder=" .. sort .. "&limit=100"
                    if cursor ~= "" then
                    ...
                    if req then
                        ...
                        if res and res.Body then result = res.Body end
                    else
                ...
                if #validServers == 0 then
                    ShowToast(TL("No available servers found.", "ไม่พบเซิร์ฟเวอร์ที่พร้อมใช้งาน"), Colors.Red)
                    return
                end
                
                local bestServer = nil
                if mode == "HighestPlayer" then
                    table.sort(validServers, function(a, b) return a.playing > b.playing end)
                    bestServer = validServers[1]
                elseif mode == "LowestPlayer" then
                    table.sort(validServers, function(a, b) return a.playing < b.playing end)
                    bestServer = validServers[1]
                elseif mode == "LowestPing" then
                    table.sort(validServers, function(a, b) return (a.ping or 9999) < (b.ping or 9999) end)
                    bestServer = validServers[1]
                elseif mode == "LowestPingAndPlayer" then
                    table.sort(validServers, function(a, b) 
                        local scoreA = (a.ping or 9999) + (a.playing * 15)
                        local scoreB = (b.ping or 9999) + (b.playing * 15)
                ...
                if bestServer then
                    ...
                    ShowToast(TL("No suitable server found.", "ไม่พบเซิร์ฟเวอร์ที่เหมาะสม"), Colors.Red)
                end
            end)
        end)
    end
    ...
    Section_ServerActions:Button({
        Name = TL("📈 Highest Player Hop", "📈 สุ่มเซิร์ฟคนเยอะ"),
        Callback = function()
            Window:Dialog({
                ...
                Buttons = {
                    {
                        Name = TL("Confirm", "ยืนยัน"),
                        Callback = function()
                            PerformServerHop("HighestPlayer")
                        end
                    },
                    { Name = TL("Cancel", "ยกเลิก") }
                }
    ...
    Section_ServerActions:Button({
        Name = TL("📶 Lowest Ping Hop", "📶 เซิร์ฟเวอร์ปิงน้อย"),
        Callback = function()
            Window:Dialog({
                Title = TL("Lowest Ping Hop", "เซิร์ฟเวอร์ปิงน้อย"),
                Description = TL(
                    "Hop to a server with the lowest ping?",
                    "คุณต้องการย้ายไปเซิร์ฟเวอร์ที่ปิงน้อยที่สุดใช่หรือไม่?"
                ),
                Buttons = {
                    {
                        Name = TL("Confirm", "ยืนยัน"),
                        Callback = function()
                            PerformServerHop("LowestPing")
                        end
                    },
                    {
                        Name = TL("Cancel", "ยกเลิก")
    ...
    Section_ServerActions:Button({
        Name = TL("⚡ Ping + Low Player Hop", "⚡ ปิงน้อย + คนน้อย"),
        Callback = function()
            Window:Dialog({
                Title = TL("Ping + Low Player Hop", "ปิงน้อย + คนน้อย"),
                Description = TL(
                    "Hop to a server with the lowest ping and low players?",
                    "คุณต้องการย้ายไปเซิร์ฟเวอร์ที่ปิงน้อยและคนน้อยใช่หรือไม่?"
                ),
                Buttons = {
                    {
                        Name = TL("Confirm", "ยืนยัน"),
                        Callback = function()
                            PerformServerHop("LowestPingAndPlayer")
                        end
                    },
                    {
                        Name = TL("Cancel", "ยกเลิก")
